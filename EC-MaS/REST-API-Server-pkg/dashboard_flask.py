...
def scaling_Horovod():
    ...
    if request.method == 'POST':
        ...
        open_job_str = "/SR-Elastic-Cluster-Framework/EC-MaS/Job-control-pkg/hosts_scripts/discover_hosts_" + str(job_index) + ".sh"
...
def tb1_gpustat_data():
    ...
    if request.method == 'POST':
        ...
        if f_gpustat:
            gpu_location_tb1 = '/SR-Elastic-Cluster-Framework/EC-MaS/Job-control-pkg/gpustat_control/' + f_gpustat.filename
...
def tb2_gpustat_data():
    ...
    if request.method == 'POST':
        f_gpustat = request.files.get('file', None)
        if f_gpustat:
            gpu_location_tb2 = '/SR-Elastic-Cluster-Framework/EC-MaS/Job-control-pkg/gpustat_control/' + f_gpustat.filename
...
def scaling_log_update():
    ...
    while True:
        open_job_str = "/SR-Elastic-Cluster-Framework/EC-MaS/Job-control-pkg/hosts_scripts/discover_hosts_" + str(job_count) + ".sh"
    ...
    for i in range(job_count):
        open_job_str = "/SR-Elastic-Cluster-Framework/EC-MaS/Job-control-pkg/hosts_scripts/discover_hosts_" + str(i) + ".sh"
        f_read = open(open_job_str, "r")
        running_gpu = f_read.read()
        running_gpu_list = running_gpu.split("\n")
        del running_gpu_list[-1]
